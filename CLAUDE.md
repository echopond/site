...
- "From above" (`#moments`, 2026-09-18; replaced "Moments") is generated by `scratch/build-page/above_gallery.py`, between `ABOVE` markers: a 78-second silent film (`media/watch/from-above.mp4`, 1080p, `preload="none"` with a poster; cut by `scratch/from-above/make_film.py` from aerial scene-splits of NS Builders' films plus Motif Media stills) and 24 Motif Media drone photographs in date order (`above-NN.jpg` and `-thumb`). `initGallery()` in `js/main.js` adds the lightbox. No identifiable faces, neighbours or plates. The Instagram cards sit below it and are hand-written. The older `gallery-NN` files are no longer used.
- "One gable, five stages" (`#gable`, 2026-09-18) is generated by `scratch/build-page/gable_sequence.py`, between `GABLE` markers: five square-on Motif Media photographs of the gable, June 2025 to January 2026, cropped from measured points (peak and base of wall) so the composition matches, always on one line (`.gallery--gable`). Two workers' faces are softened in the export (`FACES` in the script); keep that if the crops change.
