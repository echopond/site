...
- `BASE` is `https://echopond.github.io/site/` because echopond.org is still a masked forward (a frameset search engines cannot read). When the domain points at GitHub Pages directly, change `BASE`, re-run both scripts, and update `sitemap.xml`, `robots.txt` and `llms.txt`. `robots.txt` only takes effect then, since crawlers read it at the host root.
